            ...
            if (level == 1) {
                ...
                if (cache == 0 && hash_counter[hash] != 0 && (src - o > MINOFFSET || (src == o + 1 && lits >= 3 && src > 3 && source[src] == source[src - 3] && source[src] == source[src - 2] && source[src] == source[src - 1] && source[src] == source[src + 1] && source[src] == source[src + 2]))) {
                    ...
                    if (source[o + 3] != source[src + 3]) {
                        int f = 3 - 2 | (hash << 4);
                        destination[dst] = (byte) (f >>> 0);
                        destination[dst + 1] = (byte) (f >>> 8);
                        src += 3;
                        dst += 2;
                    } else {
                        int old_src = src;
                        int remaining = min((source.length - UNCOMPRESSED_END - src + 1 - 1), 255);
    ...
    static public byte[] decompress(byte[] source) {
        ...
        for (; ; ) {
            ...
            if ((cword_val & 1) == 1) {
                ...
                if (matchlen - 3 >= 0) System.arraycopy(destination, offset2 + 3, destination, dst + 3, matchlen - 3);
                dst += matchlen;
